...
void debug_print_bfs_id_bit_set(uint64_t bfs_id_bit_set) {
  _debug_mati_simple() << std::bitset<64>(bfs_id_bit_set);
}
...
void BFSMultipleStartsOptimized<MULTIPLE_FINAL>::_begin(
  ...
  _debug_mati() << "num_chunks = " << num_chunks << std::endl;
  current_bfs_chunk = 0;
...
void BFSMultipleStartsOptimized<
  ...
  for (int bfs_index = 0; bfs_index < num_nodes_in_current_chunk; bfs_index++) {
    ...
    _debug_mati() << "Adding start_node_id " << start_node_id.first << ", "
                  << start_node_id.second << " to search_states with bfs_index "
                  << bfs_index << std::endl;
    bfses_to_be_visited[start_node_id] = uint64_t(1) << bfs_index;
    _debug_mati() << "bfses_to_be_visited" << bfses_to_be_visited[start_node_id]
                  << std::endl;
...
void BFSMultipleStartsOptimized<MULTIPLE_FINAL>::set_new_bfs_chunk_and_reset() {
  _debug_mati() << "setting new bfs chunk: current_bfs_chunk "
                << current_bfs_chunk << std::endl;
  ...
  _debug_mati() << "num_nodes_in_current_chunk: " << num_nodes_in_current_chunk
                << std::endl;
  if (num_nodes_in_current_chunk == NUM_CONCURRENT_BFS) {
    bit_mask_for_current_chunk = ~0ULL;
  } else {
    bit_mask_for_current_chunk =
        (uint64_t(1) << num_nodes_in_current_chunk) - 1;
  }
  _debug_mati() << "bit_mask_for_current_chunk: " << bit_mask_for_current_chunk
                << std::endl;
...
bool BFSMultipleStartsOptimized<MULTIPLE_FINAL>::_next() {
  ...
  current_bfs_chunk++;
  if (current_bfs_chunk < num_chunks) {
    set_new_bfs_chunk_and_reset();
    return _next();
  } else {
...
template <bool MULTIPLE_FINAL>
bool BFSMultipleStartsOptimized<MULTIPLE_FINAL>::single_next() {
  ...
  while (visit_q.size() > 0) {
    ...
    _debug_mati() << "bfses_to_be_visited_next.size()"
                  << bfses_to_be_visited_next.size() << std::endl;
...
const SearchState *BFSMultipleStartsOptimized<MULTIPLE_FINAL>::expand_neighbors(
  ...
  if (iter->at_end()) {
    ...
    if (automaton.from_to_connections[current_state_id.first].size() == 0) {
      _debug_mati() << "automaton has no transitions for "
                    << current_state_id.first << std::endl;
      return nullptr;
    }
  ...
  while (!search_states_for_current_iteration.empty()) {
    _debug_mati() << "search states for current iteration not empty! "
                  ...
                  << search_states_for_current_iteration.size() << std::endl;
...
#pragma GCC unroll 64
        for (bfs_id_bit_set bitmask_it = difference; bitmask_it;
          ...
          _debug_mati() << "bitmask_it: "
                        << std::bitset<NUM_CONCURRENT_BFS>(bitmask_it)
                        << std::endl;
          int bfs_index = __builtin_ctzll(bitmask_it);
          _debug_mati() << "bfs_index: " << bfs_index << std::endl;
          assert(search_states[bfs_index].find(current_state_id) !=
                 search_states[bfs_index].end());
          auto previous =
          ...
          if (is_final) {
            if (MULTIPLE_FINAL) {
              if (reached_final[bfs_index].find(new_node_id.second.id) ==
                ...
                _debug_mati() << "For bfs_index " << bfs_index
                              << "the final state " << new_node_id.second.id
                              << " was already reached!" << std::endl;
              }
            } else {
              search_states_for_current_iteration.push(
                  &insertion_result.first->second);

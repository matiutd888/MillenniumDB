...
void MultiSourceSearchState::print(
  ...
  if (begin_at_left) {
    // the path need to be reconstructed in the reverse direction (last seen
    // goes first)
    std::vector<ObjectId> nodes;
    std::vector<ObjectId> edges;
    std::vector<bool> inverse_directions;

    for (auto current_state = this; current_state != nullptr;
    ...
    for (int_fast32_t i = nodes.size() - 2; i >= 0;
         i--) { // don't use unsigned i, will overflow

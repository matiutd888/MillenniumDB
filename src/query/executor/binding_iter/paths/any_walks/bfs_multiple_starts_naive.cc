...
void BFSMultipleStartsNaive<MULTIPLE_FINAL>::_begin(Binding &_parent_binding) {
  ...
  set_new_start_node();
  auto state_inserted =
      visited.emplace(automaton.start_state, current_start_node, nullptr, true,
                      ObjectId::get_null());
...
bool BFSMultipleStartsNaive<MULTIPLE_FINAL>::single_next() {
  ...
  if (first_next) {
    ...
    if (automaton.is_final_state[automaton.start_state]) {
      auto reached_state =
          SearchState(automaton.start_state, current_state->node_id, nullptr,
                      true, ObjectId::get_null());
      if (MULTIPLE_FINAL) {
...
template <bool MULTIPLE_FINAL>
const SearchState *BFSMultipleStartsNaive<MULTIPLE_FINAL>::expand_neighbors(
        ...
        if (automaton.is_final_state[reached_state->automaton_state]) {
          if (MULTIPLE_FINAL) {
            auto node_reached_final =
                reached_final.find(reached_state->node_id.id);
...
template <bool MULTIPLE_FINAL>
void BFSMultipleStartsNaive<MULTIPLE_FINAL>::accept_visitor(

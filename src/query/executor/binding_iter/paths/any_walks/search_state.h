...
struct DFSSearchState {
  ...
  std::unique_ptr<EdgeIter> iter =
      std::make_unique<NullIndexIterator>(); // Iterator
...
struct SearchState {
  ...
  SearchState(uint32_t automaton_state, ObjectId node_id,
              const SearchState *previous, bool inverse_direction,
              ObjectId type_id)
      : node_id(node_id), previous(previous), type_id(type_id),
        automaton_state(automaton_state), inverse_direction(inverse_direction) {
  }
  ...
  friend std::ostream &operator<<(std::ostream &os, const SearchState &state) {
    os << "SearchState: type_id(" << state.type_id << "), automaton_state("
       << state.automaton_state << "), node_id(" << state.node_id << ")";
    return os;
  }
  // For unordered set
  bool operator==(const SearchState &other) const {
  ...
  SearchState clone() const {
    return SearchState(automaton_state, node_id, previous, inverse_direction,
                       type_id);
  }
...
struct DirectionalSearchState {
  ...
  DirectionalSearchState(uint32_t automaton_state, ObjectId node_id,
                         DirectionalSearchState *previous,
                         bool inverse_direction, ObjectId type_id, bool forward)
      : node_id(node_id), previous(previous), type_id(type_id),
        automaton_state(automaton_state), inverse_direction(inverse_direction),
        forward(forward) {}
  ...
  DirectionalSearchState clone() const {
    return DirectionalSearchState(automaton_state, node_id, previous,
                                  inverse_direction, type_id, forward);
  }
...
// For unordered set
template <> struct std::hash<Paths::Any::SearchState> {
...
// For unordered set
template <> struct std::hash<Paths::Any::DirectionalSearchState> {

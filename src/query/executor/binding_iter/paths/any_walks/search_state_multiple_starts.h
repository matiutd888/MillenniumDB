...
public:
  ...
  find(std::pair<uint64_t, uint64_t>) {
    return {0, 0};
  }
...
struct MultiSourceSearchState {
  ...
  MultiSourceSearchState(uint32_t automaton_state, ObjectId node_id,
                         const MultiSourceSearchState *previous,
                         bool inverse_direction, ObjectId type_id,
                         ObjectId bfs_id)
  ...
  friend std::ostream &operator<<(std::ostream &os,
    ...
    os << "MultiSourceSearchState: bfs_id(" << state.bfs_id << "), type_id("
       << state.type_id << "), automaton_state(" << state.automaton_state
  ...
  bool operator==(const MultiSourceSearchState &other) const {
    return automaton_state == other.automaton_state &&
           node_id == other.node_id && bfs_id == other.bfs_id;
  }

  // TODO analyze
  MultiSourceSearchState clone() const {
    return MultiSourceSearchState(automaton_state, node_id, previous,
                                  inverse_direction, type_id, bfs_id);
  }
...
// For unordered set
template <> struct std::hash<Paths::Any::MultiSourceSearchState> {

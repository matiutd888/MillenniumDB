...
void BFSMultipleStartsNaiveParallel<MULTIPLE_FINAL>::_begin(
  ...
  for (auto node : start_nodes) {
    ...
    auto start_state =
        visited.emplace(automaton.start_state, start_object_id, nullptr, true,
                        ObjectId::get_null(), start_object_id);
...
bool BFSMultipleStartsNaiveParallel<MULTIPLE_FINAL>::_next() {
  ...
  while (!first_visit_q.empty()) {
    ...
    if (automaton.is_final_state[automaton.start_state]) {
      auto reached_state = MultiSourceSearchState(
          automaton.start_state, curr_first_node.second, nullptr, true,
          ObjectId::get_null(), curr_first_node.second);
      if (MULTIPLE_FINAL) {
        reached_final.insert(
            {curr_first_node.second.id, curr_first_node.second.id});
      }
      auto path_id = path_manager.set_path(
          visited.insert(reached_state).first.operator->(), path_var);
...
BFSMultipleStartsNaiveParallel<MULTIPLE_FINAL>::expand_neighbors(
    const MultiSourceSearchState &current_state) {
  // Check if this is the first time that current_state is explored
  if (iter->at_end()) {
    current_transition = 0;
    // Check if automaton state has transitions
    if (automaton.from_to_connections[current_state.automaton_state].size() ==
        0) {
      return nullptr;
    }
  ...
  while (current_transition <
    ...
    auto &transition =
        automaton.from_to_connections[current_state.automaton_state]
                                     [current_transition];

    // Iterate over records until a final state is reached
    while (iter->next()) {
      MultiSourceSearchState next_state(
          transition.to, ObjectId(iter->get_reached_node()), &current_state,
          transition.inverse, transition.type_id, current_state.bfs_id);
      ...
      if (visited_state.second) {
        ...
        if (automaton.is_final_state[reached_state->automaton_state]) {
          if (MULTIPLE_FINAL) {
            auto node_reached_final = reached_final.find(std::make_pair(
                current_state.bfs_id.id, reached_state->node_id.id));
            if (node_reached_final == reached_final.end()) {
              reached_final.insert(std::make_pair(current_state.bfs_id.id,
                                                  reached_state->node_id.id));
...
template <bool MULTIPLE_FINAL>
void BFSMultipleStartsNaiveParallel<MULTIPLE_FINAL>::accept_visitor(
